...
public class RookMoveTracker {
    ...
    public static boolean validateRook(byte[][] board, String move, boolean white) {
    ...
    private static boolean validateRookHelper(byte[][] board, int rank, int file, boolean white) {
    ...
    public static List<String> possibleMoves(byte[][] board, int rank, int file, boolean white) {
        ...
        for (int d = 0; d < 4; d++) {
            ...
            while (isValidSquare(rank + i * dy[d], file + i * dx[d])) {
                ...
                if (!white && squareContent < 0) break;
                if (squareContent == 0) {
                    copy[rank + i * dy[d]][file + i * dx[d]] = (byte) (white ? 5 : -5);
                    copy[rank][file] = 0;
                    ...
                    copy[rank + i * dy[d]][file + i * dx[d]] = 5;
                    copy[rank][file] = 0;
    ...
    private static boolean validateRookHelper(byte[][] board, int rank, int file, boolean white, int myFile, int myRank) {
        for (int d = 0; d < 4; d++) {
            int i = 1;
            while (isValidSquare(rank + i * dy[d], file + i * dx[d])) {
                byte squareContent = board[rank + i * dy[d]][file + i * dx[d]];
                if (squareContent == 5 && white) {
